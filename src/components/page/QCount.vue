...
<el-button type="text" @click="dialogFormVisible = true">新增收支</el-button>
 总计：{{count}} 收入 
</span> 
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getData(page,size){
              let self = this;
              self.$axios.get(self.url,{params: { 'page':page,'size':size,account_type:'qxy'  }}).then((res) => {
            ...
            handleDelete(index, row, data) {
              ...
              this.$axios.get(this.url+"/del/"+row.id).then((res) => {
